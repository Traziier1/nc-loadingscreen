...
function setup() {
    ...
    Config.Staff.forEach((member, index) => {
        $(".staff .innercards").append(`<div class="card" data-id="${index}" style="--color: ${member.color}">
            ...
            <p style = "text-align:center" class="quote">"${member.quote}"</p>
    ...
    $(".staff .next").on("click", function() {
        ...
        if (currentPage < Config.Staff.length - 2) {
            ...
            currentPage = 0; // Go back to the beginning when the end is reached
        }
    ...
    // Auto cycle through carousel
    var autoCycleInterval = setInterval(function () {
        $(".staff .next").trigger("click");
    }, 6000);

    // Pause auto cycle on hover
    $(".staff .innercards").hover(
        function () {
